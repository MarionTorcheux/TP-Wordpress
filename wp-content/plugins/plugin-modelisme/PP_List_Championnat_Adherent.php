...
class PP_List_Championnat_Adherent extends WP_List_Table
{
    private $dal;
    ...
    {
        parent::__construct([
            'singular' => __('Championnat Adherent'),
            'plural' => __('Championnats Adherents'),
        ]);
    ...
    public function prepare_items()
    {
        ...
        $perPage = $this->get_items_per_page('championnat_adherent_per_page', 10);
        $currentPage = $this->get_pagenum(); // permet de savoir sur quelle page on est

        // on traite les données
        $data = $this->dal->findAllChampionnats_adherent(); // pour récupérer les infos de la bdd
    ...
    public function get_sortable_columns()
    {
        $sortable = [
            ...
            'adherent_ID' => array('adherent_ID', true),
        ];
        return $sortable;
    ...
    {       // convertir l'élément en tableau
        ...
        return sprintf(
            "<input type='checkbox' name='delete-championnat-adherent[]' value='%s' />", $item['ID']
        );
    }
    ...
    {
        return array(
            'delete-championnat-adherent' => __('Delete'),
            'update-championnat-adherent' => __('Update')
        );
    }
